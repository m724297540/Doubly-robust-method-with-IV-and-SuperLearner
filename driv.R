...
setwd("C:/Users/cheng/Dropbox/research/chns")
set.seed(123456)
...
result <- driv.final(Y,X,T,Z,k=5, "lm")
datanew <-  result$cate
haz_boy_lm <- result$lm
data <- cbind(data, datanew)
data <- rename(data, haz_boy_lm = datanew)
write_dta(data, "data/ml_data/outcome/haz_boy_driv.dta")
